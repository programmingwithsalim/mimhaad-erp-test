...
import { Package, TrendingUp, DollarSign, Users, RefreshCw, Plus, Download, AlertTriangle } from "lucide-react"
import { format } from "date-fns"

export default function JumiaPage() {
  const { toast } = useToast()
  const { user } = useCurrentUser()
  const { statistics, floatAlerts, isLoading: statsLoading, refreshStatistics } = useServiceStatistics("jumia")
  ...
  const loadTransactions = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/jumia/transactions?branchId=${user.branchId}&limit=50`)
      ...
      if (response.ok) {
        ...
        if (data.success && Array.isArray(data.accounts)) {
          ...
          const activeAccounts = data.accounts.filter(
            (account: any) => account.is_active && account.account_type !== "cash-in-till",
          )
    ...
    try {
      ...
      if (result.success) {
        toast({
          ...
          description: `Payment of ${formatCurrency(Number.parseFloat(podForm.amount))} collected for tracking ID ${podForm.tracking_id}.`,
        })
        setPodForm({
          tracking_id: "",
          amount: "",
    ...
    try {
      ...
      if (result.success) {
        toast({
          ...
          description: `Settlement of ${formatCurrency(Number.parseFloat(settlementForm.amount))} has been successfully processed.`,
        })
        setSettlementForm({
          amount: "",
          reference: "",
  ...
  const exportToCSV = () => {
    const headers = ["Date", "Type", "Tracking ID", "Customer", "Phone", "Amount", "Status", "Payment Method"]
    ...
    link.setAttribute("download", `jumia-transactions-${format(new Date(), "yyyy-MM-dd")}.csv`)
    ...
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Manage packages, collections, and settlements</p>
        </div>
        <Button
          variant="outline"
          onClick={() => {
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Today's Packages</CardTitle>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Unsettled Amount</CardTitle>
            <Users className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{formatCurrency(statistics.floatBalance)}</div>
            ...
            <div className="lg:col-span-2">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Manage packages, collections, and settlements</CardDescription>
                </CardHeader>
                <CardContent>
                  <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                    <TabsList className="grid w-full grid-cols-3">
                      <TabsTrigger value="package_receipt">Packages</TabsTrigger>
                      <TabsTrigger value="pod_collection">Collections</TabsTrigger>
                      <TabsTrigger value="settlement">Settlements</TabsTrigger>
                    </TabsList>

                    <TabsContent value="package_receipt" className="space-y-4 mt-4">
                        ...
                        <div className="grid gap-4 md:grid-cols-2">
                          <div className="space-y-2">
                            ...
                            <Input
                              ...
                              onChange={(e) => setPackageForm({ ...packageForm, tracking_id: e.target.value })}
                          ...
                          <div className="space-y-2">
                            ...
                            <Input
                              ...
                              onChange={(e) => setPackageForm({ ...packageForm, customer_name: e.target.value })}
                          ...
                          <div className="space-y-2">
                            ...
                            <Input
                              ...
                              onChange={(e) => setPackageForm({ ...packageForm, customer_phone: e.target.value })}
                    ...
                    <TabsContent value="pod_collection" className="space-y-4 mt-4">
                        ...
                        <div className="grid gap-4 md:grid-cols-2">
                          <div className="space-y-2">
                            ...
                            <Input
                              ...
                              onChange={(e) => setPodForm({ ...podForm, tracking_id: e.target.value })}
                              placeholder="Enter tracking ID"
                              required
                            />
                          </div>
                          ...
                          <div className="space-y-2">
                            <Label htmlFor="pod_amount">Amount to Collect (GHS)</Label>
                            <Input
                              id="pod_amount"
                              type="number"
                              step="0.01"
                              ...
                              onChange={(e) => setPodForm({ ...podForm, amount: e.target.value })}
                              placeholder="0.00"
                              required
                            />
                          </div>
                          ...
                          <div className="space-y-2">
                            ...
                            <Input
                              ...
                              onChange={(e) => setPodForm({ ...podForm, customer_name: e.target.value })}
                              placeholder="Enter customer name"
                              required
                            />
                          </div>
                          ...
                          <div className="space-y-2">
                            ...
                            <Input
                              ...
                              onChange={(e) => setPodForm({ ...podForm, customer_phone: e.target.value })}
                              placeholder="Enter phone number"
                              required
                            />
                          </div>
                          ...
                          <div className="space-y-2">
                            ...
                            <Select
                              ...
                              onValueChange={(value) => setPodForm({ ...podForm, delivery_status: value })}
                            >
                              <SelectTrigger>
                          ...
                          <div className="space-y-2">
                            ...
                            <Select
                              ...
                              onValueChange={(value) => setPodForm({ ...podForm, payment_method: value })}
                            >
                              <SelectTrigger>
                    ...
                    <TabsContent value="settlement" className="space-y-4 mt-4">
                      <form onSubmit={handleSettlementSubmit} className="space-y-4">
                        <div className="grid gap-4 md:grid-cols-2">
                          <div className="space-y-2">
                            <Label htmlFor="settlement_amount">Settlement Amount (GHS)</Label>
                            <Input
                          ...
                          <div className="space-y-2">
                            <Label htmlFor="settlement_reference">Reference</Label>
                            <Input
                          ...
                          <div className="space-y-2">
                            <Label htmlFor="settlement_float_account">Float Account</Label>
                            <Select
                              value={settlementForm.float_account_id}
                              onValueChange={(value) =>
                                setSettlementForm({ ...settlementForm, float_account_id: value })
                              }
                            >
                              <SelectTrigger>
                              ...
                              <SelectContent>
                                {floatAccounts.map((account: any) => (
                                  <SelectItem key={account.id} value={account.id}>
                                    {account.provider} - {formatCurrency(account.current_balance)}
                                  </SelectItem>
                                ))}
                              </SelectContent>
                            </Select>
                        ...
                        <div className="space-y-2">
                          <Label htmlFor="settlement_notes">Notes (Optional)</Label>
                          <Textarea
                            id="settlement_notes"
            ...
            <CardContent>
              {loadingTransactions ? (
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-muted-foreground">No Jumia transactions have been processed yet.</p>
                </div>
              ) : (
                <Table>
                  <TableHeader>
                  ...
                  <TableBody>
                    {transactions.map((transaction: any) => (
                      <TableRow key={transaction.id}>
                        ...
                        <TableCell>{getStatusBadge(transaction.delivery_status || transaction.status)}</TableCell>
                        <TableCell className="capitalize">
