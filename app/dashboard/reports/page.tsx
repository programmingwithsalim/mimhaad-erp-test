...
export default function ReportsPage() {
    ...
    try {
      ...
      if (result.success) {
        ...
        const [
          ...
          fetch(
            `/api/reports/cash-flow?from=${format(
              dateRange.from,
              "yyyy-MM-dd"
            )}&to=${format(
              dateRange.to,
          ...
          fetch(
            `/api/reports/balance-sheet?from=${format(
              dateRange.from,
              "yyyy-MM-dd"
            )}&to=${format(
              dateRange.to,
        ...
        const cashFlowResult = await cashFlowData.json();
        const balanceSheetResult = await balanceSheetData.json();

        console.log("🏢 Fixed Assets result:", fixedAssetsResult);
        console.log("💰 Expenses result:", expensesResult);
        console.log("📈 Equity result:", equityResult);
        console.log("📊 Profit Loss result:", profitLossResult);
        console.log("💸 Cash Flow result:", cashFlowResult);
        console.log("📋 Balance Sheet result:", balanceSheetResult);

        // Debug profit-loss specifically
        console.log("🔍 Profit Loss Debug:");
        console.log("  - success:", profitLossResult.success);
        console.log("  - data exists:", !!profitLossResult.data);
        console.log("  - summary exists:", !!profitLossResult.data?.summary);
        console.log(
          "  - totalRevenue:",
          profitLossResult.data?.summary?.totalRevenue
        );
        console.log("  - full data:", profitLossResult.data);
        ...
        const reportDataToSet = {
          ...
          cashFlow: cashFlowResult.success ? cashFlowResult.data : null,
          balanceSheet: balanceSheetResult.success
        ...
        console.error("❌ Comprehensive report failed:", result.error);
  ...
  const drawBalanceSheetContent = (
    ...
    page.drawText(
      `GHS ${data.assets.current.cashAndCashEquivalents.toLocaleString()}`,
      {
        x: 400,
        y,
        size: 10,
    ...
    page.drawText(
      `GHS ${data.assets.current.accountsReceivable.toLocaleString()}`,
      {
        x: 400,
        y,
        size: 10,
    ...
    page.drawText(`GHS ${data.assets.fixed.netFixedAssets.toLocaleString()}`, {
      x: 400,
      y,
      size: 10,
      font: font,
    ...
    page.drawText(`GHS ${data.assets.total.toLocaleString()}`, {
      x: 400,
      y,
      size: 12,
      font: boldFont,
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="space-y-3">
                        <div className="flex justify-between items-center py-3 border-b border-gray-100">
                          ...
                          <span className="font-semibold text-green-600">
                            GHS{" "}
                            {reportData.balanceSheet.assets.current.cashAndCashEquivalents.toLocaleString()}
                          </span>
                        </div>
                        <div className="flex justify-between items-center py-3 border-b border-gray-100">
                          <span className="font-medium">
                        ...
                        <div className="flex justify-between items-center py-4 border-t-2 border-green-200 font-bold text-lg">
                          ...
                          <span className="text-green-600">
                            GHS{" "}
                            {reportData.balanceSheet.assets.current.totalCurrent.toLocaleString()}
                          </span>
                        </div>
                        <div className="flex justify-between items-center py-3 border-b border-gray-100">
                          <span className="font-medium">
                          ...
                          <span className="font-semibold text-green-600">
                            GHS{" "}
                            {reportData.balanceSheet.assets.fixed.netFixedAssets.toLocaleString()}
                          </span>
                        </div>
                        <div className="flex justify-between items-center py-4 border-t-2 border-green-200 font-bold text-lg">
                          ...
                          <span className="text-green-600">
                            GHS{" "}
                            {reportData.balanceSheet.assets.total.toLocaleString()}
                          </span>
                        </div>
                      </div>
                    </div>
                      ...
                      <div className="space-y-3">
                        <div className="flex justify-between items-center py-3 border-b border-gray-100">
                          ...
                          <span className="font-semibold text-red-600">
                            GHS{" "}
                            {reportData.balanceSheet.liabilities.current.accountsPayable.toLocaleString()}
                          </span>
                        </div>
                        <div className="flex justify-between items-center py-4 border-t-2 border-red-200 font-bold text-lg">
                          ...
                          <span className="text-red-600">
                            GHS{" "}
                            {reportData.balanceSheet.liabilities.total.toLocaleString()}
                          </span>
                        </div>
                      </div>
                    </div>
                      ...
                      <div className="space-y-3">
                        <div className="flex justify-between items-center py-3 border-b border-gray-100">
                          ...
                          <span className="font-semibold text-blue-600">
                            GHS{" "}
                            {reportData.balanceSheet.equity.retainedEarnings.toLocaleString()}
                          </span>
                        </div>
                        <div className="flex justify-between items-center py-4 border-t-2 border-blue-200 font-bold text-lg">
                          ...
                          <span className="text-blue-600">
                            GHS{" "}
                            {reportData.balanceSheet.equity.totalEquity.toLocaleString()}
                          </span>
                        </div>
                      </div>
                    </div>
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  ...
                  <Button
                    variant="outline"
                    onClick={() =>
                      exportReportPDF("Profit & Loss", reportData?.profitLoss)
                    }
                  >
                    <Download className="h-4 w-4 mr-2" />
                    Export PDF
                ...
                {reportData?.profitLoss ? (
                  <div className="space-y-8">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="space-y-3">
                        {reportData.profitLoss.revenue.breakdown.map(
                          (service: any, index: number) => (
                            ...
                            >
                              ...
                              <span className="font-semibold text-green-600">
                                GHS {service.revenue.toLocaleString()}
                              </span>
                            </div>
                          )
                        )}
                        <div className="flex justify-between items-center py-4 border-t-2 border-green-200 font-bold text-lg">
                          ...
                          <span className="text-green-600">
                            GHS{" "}
                            {reportData.profitLoss.revenue.total.toLocaleString()}
                          </span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="space-y-3">
                        {reportData.profitLoss.expenses.breakdown.map(
                        ...
                        <div className="flex justify-between items-center py-4 border-t-2 border-red-200 font-bold text-lg">
                          ...
                          <span className="text-red-600">
                            GHS{" "}
                            {reportData.profitLoss.expenses.total.toLocaleString()}
                          </span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="space-y-3">
                        <div className="flex justify-between items-center py-3 border-b border-gray-100">
                          ...
                          <span className="font-semibold text-blue-600">
                            GHS{" "}
                            {reportData.profitLoss.profitLoss.grossProfit.toLocaleString()}
                          </span>
                        </div>
                        <div className="flex justify-between items-center py-3 border-b border-gray-100">
                          ...
                          <span className="font-semibold text-blue-600">
                            GHS{" "}
                            {reportData.profitLoss.commissions.total.toLocaleString()}
                          </span>
                        </div>
                        <div className="flex justify-between items-center py-4 border-t-2 border-blue-200 font-bold text-lg">
                          ...
                          <span className="text-blue-600">
                            GHS{" "}
                            {reportData.profitLoss.profitLoss.netIncome.toLocaleString()}
                          </span>
                        </div>
                        <div className="flex justify-between items-center py-3 border-b border-gray-100">
                          <span className="font-medium">Profit Margin</span>
                          <span className="font-semibold text-blue-600">
                            {reportData.profitLoss.profitLoss.profitMargin.toFixed(
                              2
                            )}
                            %
                          </span>
                  ...
                  <div className="text-center py-12">
                    ...
                    <p className="text-muted-foreground">
                      No transaction data available for profit & loss statement.
                    </p>
                    {/* Debug information */}
                    <div className="mt-4 p-4 bg-gray-100 rounded-lg text-left">
                      <h4 className="font-semibold mb-2">Debug Info:</h4>
                      <p className="text-sm">
                        reportData exists: {reportData ? "Yes" : "No"}
                      </p>
                      <p className="text-sm">
                        profitLoss exists:{" "}
                        {reportData?.profitLoss ? "Yes" : "No"}
                      </p>
                      <p className="text-sm">
                        profitLoss data:{" "}
                        {JSON.stringify(reportData?.profitLoss, null, 2)}
                      </p>
                    </div>
                  </div>
                )}
          ...
          <TabsContent value="equity" className="space-y-6">
            <Card>
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div>
                    <CardTitle>Statement of Equity</CardTitle>
                    <CardDescription>
