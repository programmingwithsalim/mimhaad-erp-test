...
export default function MoMoPage() {
  ...
  const { statistics, floatAlerts, isLoading: statsLoading, refreshStatistics } = useServiceStatistics("momo")
  ...
  const loadTransactions = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/momo/transactions?branchId=${user.branchId}&limit=50`)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.type || !formData.amount || !formData.customer_name || !formData.phone_number || !formData.provider) {
      toast({
        title: "Error",
  ...
  const exportToCSV = () => {
    const headers = ["Date", "Type", "Customer", "Phone", "Provider", "Amount", "Fee", "Status"]
    ...
    link.setAttribute("download", `momo-transactions-${format(new Date(), "yyyy-MM-dd")}.csv`)
    ...
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Manage mobile money transactions and float accounts</p>
        </div>
        <Button
          variant="outline"
          onClick={() => {
            ...
            <CardHeader>
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <CardDescription>All mobile money transactions</CardDescription>
                </div>
            ...
            <CardContent>
              {loadingTransactions ? (
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-muted-foreground">No mobile money transactions have been processed yet.</p>
                </div>
              ) : (
                <Table>
                  <TableHeader>
            ...
            <div id="receipt-content" className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <div className="flex justify-between text-sm font-medium border-t pt-2">
                  ...
                  <span>
                    {formatCurrency(
                      currentTransaction.amount + (currentTransaction.type === "cash-out" ? currentTransaction.fee : 0),
                    )}
                  </span>
                </div>
              </div>
