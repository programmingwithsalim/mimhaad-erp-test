...
export async function POST(request: Request) {
  try {
    ...
    const { fromAccountId, toAccountId, amount, reference, processedBy, userId } = body
    ...
    if (fromBalance < amount) {
      return NextResponse.json({ success: false, error: "Insufficient balance in source account" }, { status: 400 })
    }
    ...
    try {
      ...
      await sql`
        ...
        (${fromAccountId}, 'transfer_out', ${-amount}, 'Transfer to partner account', ${reference}, ${processedBy}, NOW()),
        (${toAccountId}, 'transfer_in', ${amount}, 'Transfer from E-Zwich settlement', ${reference}, ${processedBy}, NOW())
      `
      ...
      return NextResponse.json({
        success: true,
        message: `Successfully transferred GHS ${amount.toFixed(2)} to partner account`,
        transfer: {
          amount,
