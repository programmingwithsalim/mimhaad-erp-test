...
export async function GET(request: NextRequest) {
  try {
    ...
    currentAssets.totalCurrent = Object.values(currentAssets).reduce((sum, val) => sum + val, 0)

    const nonCurrentAssets = {
      propertyPlantEquipment: Number(assets.property_plant_equipment || 0),
      accumulatedDepreciation: -Number(assets.property_plant_equipment || 0) * 0.1, // Simplified depreciation
      netPropertyPlantEquipment: Number(assets.property_plant_equipment || 0) * 0.9,
    ...
    const currentLiabilities = {
      ...
      currentPortionLongTermDebt: Number(liabilities.current_portion_long_term_debt || 0),
      taxesPayable: Number(liabilities.taxes_payable || 0),
    }
    currentLiabilities.totalCurrent = Object.values(currentLiabilities).reduce((sum, val) => sum + val, 0)
    ...
    nonCurrentLiabilities.totalNonCurrent = Object.values(nonCurrentLiabilities).reduce((sum, val) => sum + val, 0)

    const equityData_final = {
      capitalStock: Number(equity.capital_stock || 1000000),
      retainedEarnings: Number(equity.retained_earnings || 0),
      accumulatedOtherComprehensiveIncome: Number(equity.accumulated_other_comprehensive_income || 0),
    }
    equityData_final.totalEquity = Object.values(equityData_final).reduce((sum, val) => sum + val, 0)

    const balanceSheetData = {
      assets: {
        current: currentAssets,
        nonCurrent: nonCurrentAssets,
        totalAssets: currentAssets.totalCurrent + nonCurrentAssets.totalNonCurrent,
      },
      liabilities: {
        current: currentLiabilities,
        nonCurrent: nonCurrentLiabilities,
        totalLiabilities: currentLiabilities.totalCurrent + nonCurrentLiabilities.totalNonCurrent,
      },
