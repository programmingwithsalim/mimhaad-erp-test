...
export async function POST(request: Request) {
  try {
    ...
    console.log("💰 [RECHARGE] Processing recharge:", JSON.stringify(requestData, null, 2))

    const { accountId, amount, paymentAccountId, description, performedBy } = requestData
    ...
    if (paymentAccountId) {
      ...
      if (paymentBalance < rechargeAmount) {
        return NextResponse.json(
          {
            success: false,
            error: `Insufficient balance in payment account. Available: GHS ${paymentBalance.toFixed(2)}, Required: GHS ${rechargeAmount.toFixed(2)}`,
          },
          { status: 400 },
        )
      }
      ...
      await sql`
        INSERT INTO float_transactions (
          ...
          ${description || `Transfer to ${targetAccount[0].account_type} account`},
          ${performedBy},
