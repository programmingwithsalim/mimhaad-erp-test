...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (account.length === 0) {
      return NextResponse.json({ error: `Account with ID ${params.id} not found` }, { status: 404 })
    }
...
export async function PUT(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const { account_code, account_name, account_type, parent_id, is_active } = body
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (journalTableExists[0].exists) {
      ...
      if (columnNames.includes("debit_account_id") && columnNames.includes("credit_account_id")) {
