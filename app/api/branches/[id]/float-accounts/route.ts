...
export async function GET(
  ...
  try {
    ...
    // RBAC: Only allow Admin, Manager, or Finance for their own branch
    const user = await getCurrentUser(request);
    console.log("Current user:", { id: user.id, role: user.role, branchId: user.branchId });
    ...
    console.log("RBAC check:", { isAdmin, isManager, isFinanceOwnBranch, userBranchId: user.branchId, requestedBranchId: branchId });
    
    if (!isAdmin && !isManager && !isFinanceOwnBranch) {
      return NextResponse.json(
