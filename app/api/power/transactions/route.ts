...
export async function POST(request: NextRequest) {
  try {
    ...
    const transactionId = `pwr_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`
    ...
    await sql`
      INSERT INTO power_transactions (
        ...
        ${transactionId}, ${body.meter_number}, ${body.amount}, ${body.customer_name},
        ${body.customer_phone || null}, ${body.provider}, ${body.reference || transactionId},
        'completed', NOW(), ${body.branchId}, ${body.userId}, ${body.notes || null}
      )
    `
        ...
        entries: [
          {
            ...
            metadata: { transactionId, provider: body.provider, meterNumber: body.meter_number },
          },
          {
