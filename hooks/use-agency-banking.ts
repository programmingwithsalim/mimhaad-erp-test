...
export function useAgencyBanking(branchId: string) {
  const [floatAccount, setFloatAccount] = useState<AgencyBankingAccount | null>(null)
  ...
  const fetchAccount = async () => {
    try {
      ...
      // Fetch float account
      const accountResponse = await fetch(`/api/branches/${branchId}/float-accounts?accountType=agency-banking`)
      if (!accountResponse.ok) {
        throw new Error(`Failed to fetch agency banking account: ${accountResponse.status}`)
      }
      ...
      // Fetch statistics
      const statsResponse = await fetch(`/api/agency-banking/statistics?branchId=${branchId}`)
      if (!statsResponse.ok) {
        throw new Error(`Failed to fetch agency banking statistics: ${statsResponse.status}`)
      }
