...
export function useMoMoTransactions({ branchId, provider }: UseMoMoTransactionsProps = {}) {
  ...
  const fetchTransactions = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || `Failed to fetch transactions: ${response.status}`)
      }
      ...
      if (Array.isArray(data.transactions)) {
        ...
        console.warn("Expected transactions to be an array but got:", data.transactions)
        setTransactions([])
  ...
  const createTransaction = async (
    transactionData: Omit<MoMoTransaction, "id" | "date" | "status" | "branchName" | "floatAccountName">,
  ) => {
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || `Failed to create transaction: ${response.status}`)
      }
      ...
      toast({
        ...
        description: `${transactionData.type === "cash-in" ? "Cash-In" : "Cash-Out"} transaction for ${transactionData.customerName} was processed successfully.`,
      })
