...
export function useServiceStatistics(serviceEndpoint: string) {
  ...
  const fetchStatistics = async () => {
    ...
    try {
      ...
      const statsResponse = await fetch(`/api/${serviceEndpoint}/statistics?branchId=${user.branchId}`)
      ...
      // Fetch float alerts
      const alertsResponse = await fetch(`/api/float-accounts?branchId=${user.branchId}&alerts=true`)
        ...
        if (alertsData.success && Array.isArray(alertsData.accounts)) {
          const alerts = alertsData.accounts
            ...
            .map((account: any) => ({
              ...
              severity:
                Number(account.current_balance || 0) < Number(account.min_threshold || 0) * 0.5
                  ? "critical"
                  : "warning",
            }))
