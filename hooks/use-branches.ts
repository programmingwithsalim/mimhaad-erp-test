...
export function useBranches() {
  ...
  const [statistics, setStatistics] = useState<BranchStatistics>(defaultStatistics)
  ...
  const searchBranches = useCallback(
    async (query: string): Promise<Branch[]> => {
      ...
      try {
        ...
        setError(err instanceof Error ? err.message : "Failed to search branches")
        setBranches([])
        toast({
          title: "Error",
    ...
    async (branchData: Omit<Branch, "id" | "created_at" | "updated_at">) => {
      try {
        ...
        if (!response.ok) {
          ...
          throw new Error(errorData.error || `Error creating branch: ${response.statusText}`)
        }
        ...
        toast({
          title: "Error",
          description: err instanceof Error ? err.message : "Failed to create branch. Please try again.",
          variant: "destructive",
        })
        throw err
      }
  ...
  const updateBranch = useCallback(
    async (id: string, branchData: Partial<Omit<Branch, "id" | "created_at" | "updated_at">>) => {
      try {
        ...
        if (!response.ok) {
          const errorData = await response.json()
          throw new Error(errorData.error || `Error updating branch: ${response.statusText}`)
        }
        ...
        toast({
          title: "Error",
          description: err instanceof Error ? err.message : "Failed to update branch. Please try again.",
          variant: "destructive",
        })
        throw err
      }
  ...
  const deleteBranch = useCallback(
    async (id: string) => {
      try {
        ...
        if (!response.ok) {
          const errorData = await response.json()
          throw new Error(errorData.error || `Error deleting branch: ${response.statusText}`)
        }
        ...
        toast({
          title: "Error",
          description: err instanceof Error ? err.message : "Failed to delete branch. Please try again.",
          variant: "destructive",
        })
        throw err
      }
