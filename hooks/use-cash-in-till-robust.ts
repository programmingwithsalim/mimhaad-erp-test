...
export function useCashInTillRobust(): UseCashInTillReturn {
  const [cashAccount, setCashAccount] = useState<CashInTillAccount | null>(null)
  ...
  const fetchCashInTill = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/float-accounts?branchId=${user.branchId}&type=cash-in-till`, {
        cache: "no-store",
        headers: {
      ...
      if (data.success && data.accounts && data.accounts.length > 0) {
        ...
        if (cashTillAccount) {
          setCashAccount(cashTillAccount)
        } else {
      ...
      setError(err instanceof Error ? err.message : "Failed to fetch cash in till")
  ...
  const updateBalance = useCallback(
    async (amount: number, description?: string) => {
      ...
      try {
        const response = await fetch(`/api/float-accounts/${cashAccount.id}/update-balance`, {
          method: "POST",
          headers: {
  ...
  const isLowBalance = cashAccount ? cashAccount.current_balance < cashAccount.min_threshold : false
