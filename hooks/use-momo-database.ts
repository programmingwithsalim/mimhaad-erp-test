...
export function useMoMoDatabase() {
  ...
  const [cashTillAccount, setCashTillAccount] = useState<MoMoFloatAccount | null>(null)
  ...
  const fetchMoMoAccounts = useCallback(async () => {
    try {
      const response = await fetch(`/api/momo/branch/${CURRENT_BRANCH_ID}/accounts`)
      if (!response.ok) {
      ...
      const missingProviders = expectedProviders.filter((provider) => !fetchedProviders.includes(provider))
      ...
      setError(err instanceof Error ? err.message : "Failed to fetch MoMo accounts")
  ...
  const fetchCashTillAccount = useCallback(async () => {
    try {
      const response = await fetch(`/api/momo/branch/${CURRENT_BRANCH_ID}/cash-till`)
      if (!response.ok) {
        const errorData = await response.json()
      ...
      setError(err instanceof Error ? err.message : "Failed to fetch cash till account")
    }
  }, [])
  ...
  const fetchTransactions = useCallback(
    async (filters?: {
      ...
      try {
        ...
        const response = await fetch(`/api/momo/branch/${CURRENT_BRANCH_ID}/transactions?${params}`)
        if (!response.ok) {
          const errorData = await response.json()
  ...
  const fetchStatistics = useCallback(async () => {
    try {
      const response = await fetch(`/api/momo/branch/${CURRENT_BRANCH_ID}/statistics`)
      if (!response.ok) {
        const errorData = await response.json()
  ...
  const createTransaction = useCallback(
    async (transactionData: {
      ...
      try {
        ...
        const response = await fetch(`/api/momo/branch/${CURRENT_BRANCH_ID}/transactions`, {
          method: "POST",
          headers: {
        ...
        if (!response.ok) {
          const errorData = await response.json()
          throw new Error(errorData.error || errorData.details || "Failed to create transaction")
        }

        const data = await response.json()
        const newTransaction = data.transaction

        const floatAccount = momoAccounts.find((acc) => acc.id === transactionData.float_account_id)
        ...
        const completeTransaction: MoMoTransaction = {
          ...
          float_account_name: `${provider} - ${floatAccount?.branch_name || "Unknown Branch"}`,
            ...
            if (account.id === transactionData.float_account_id) {
              return {
                ...
                current_balance:
                  ...
                  (transactionData.type === "cash-in" ? -transactionData.amount : transactionData.amount),
              }
            }
            return account
          }),
        ...
        setStatistics((prevStats) => ({
          ...
          cashInCount: prevStats.cashInCount + (transactionData.type === "cash-in" ? 1 : 0),
          cashOutCount: prevStats.cashOutCount + (transactionData.type === "cash-out" ? 1 : 0),
          todayTransactions: prevStats.todayTransactions + 1,
          todayVolume: prevStats.todayVolume + transactionData.amount,
        }))

        setTimeout(() => {
          Promise.all([fetchMoMoAccounts(), fetchCashTillAccount(), fetchTransactions(), fetchStatistics()]).catch(
            (err) => console.error("Error refreshing data after transaction:", err),
          )
        }, 1000)

        toast({
          title: "Transaction Successful",
          description: `${transactionData.type === "cash-in" ? "Cash-In" : "Cash-Out"} transaction processed successfully.`,
        })
    ...
    [momoAccounts, cashTillAccount, fetchMoMoAccounts, fetchCashTillAccount, fetchTransactions, fetchStatistics, toast],
  )
  ...
  const refreshData = useCallback(async () => {
    ...
    try {
      await Promise.all([fetchMoMoAccounts(), fetchCashTillAccount(), fetchTransactions(), fetchStatistics()])
    } catch (err) {
  ...
  }, [fetchMoMoAccounts, fetchCashTillAccount, fetchTransactions, fetchStatistics])
