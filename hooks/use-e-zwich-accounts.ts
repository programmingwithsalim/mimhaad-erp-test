...
export function useEZwichAccounts() {
  ...
  const fetchAccounts = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/e-zwich/accounts?branchId=${encodeURIComponent(user.branchId)}`)
      ...
      setError(err instanceof Error ? err.message : "Failed to fetch E-Zwich accounts")
      setAccounts([])
    } finally {
      setLoading(false)
    }
