...
export function useFloat({ branchId, serviceType, userId, provider }: UseFloatProps) {
  ...
  const fetchFloatAccount = useCallback(async () => {
    ...
    if (!branchId) {
      console.log("No branchId provided, skipping float account fetch")
      setIsLoading(false)
      return
    }
    ...
    try {
      ...
      console.log("Fetching float account for:", { branchId, serviceType, provider })
      ...
      console.log("Float accounts API response:", data)
      ...
      const account = data.floatAccounts?.find((acc: any) => {
        const matchesBranch = acc.branch_id === branchId || acc.branchId === branchId
        const matchesType = acc.account_type === serviceType || acc.accountType === serviceType
      ...
      if (account) {
        ...
        const normalizedAccount: FloatAccount = {
          ...
          currentBalance: Number(account.current_balance || account.currentBalance || 0),
          minThreshold: Number(account.min_threshold || account.minThreshold || 0),
          maxThreshold: Number(account.max_threshold || account.maxThreshold || 100000),
          isActive: account.is_active !== false && account.isActive !== false,
          lastUpdated: account.updated_at || account.lastUpdated || new Date().toISOString(),
        ...
        setIsLowFloat(normalizedAccount.currentBalance <= normalizedAccount.minThreshold)

        console.log("Float account loaded successfully:", normalizedAccount)
      } else {
        console.warn(
          `No active ${serviceType} float account found for branch ${branchId}${provider ? ` and provider ${provider}` : ""}`,
        )
      ...
      const errorMessage = err instanceof Error ? err.message : "Failed to fetch float account"
  ...
  const processFloatTransaction = useCallback(
    async (amount: number, type: "allocation" | "return" | "transfer" | "adjustment", description: string) => {
      if (!floatAccount) {
        throw new Error(
          `No active ${serviceType} float account found for branch ${branchId}${provider ? ` and provider ${provider}` : ""}`,
        )
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(`Failed to process float transaction: ${response.statusText}`)
        }
        ...
        if (data.success) {
          ...
          const newBalance =
            type === "allocation" ? floatBalance + amount : type === "return" ? floatBalance - amount : floatBalance
          ...
          throw new Error(data.message || "Failed to process float transaction")
        }
      } catch (err) {
  ...
  const requestFloatAllocation = useCallback(
    async (
      ...
      try {
        ...
        if (!response.ok) {
          throw new Error(`Failed to request float allocation: ${response.statusText}`)
        }
