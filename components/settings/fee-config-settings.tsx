...
const feeConfigSchema = z.object({
  ...
  fee_type: z.enum(["fixed", "percentage"], { message: "Fee type is required" }),
...
export function FeeConfigSettings({ userRole = "admin" }: { userRole?: string }) {
  ...
  const onSubmit = async (data: FeeConfigFormValues) => {
    try {
      ...
      const url = editingConfig ? `/api/settings/fee-config/${editingConfig.id}` : "/api/settings/fee-config"
      ...
      if (response.ok && result.success) {
        toast({
          title: "Success",
          description: `Fee configuration ${editingConfig ? "updated" : "created"} successfully`,
        })
        form.reset()
  ...
  const canManageFees = userRole?.toLowerCase() === "admin" || userRole?.toLowerCase() === "finance"
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              <CardTitle>Fee Configuration</CardTitle>
              <CardDescription>Manage transaction fees for different services</CardDescription>
            </div>
            {canManageFees && (
              <Button
                onClick={() => {
        ...
        <CardContent>
          {!canManageFees ? (
            <Alert>
              ...
              <AlertDescription>
                You don't have permission to manage fee configurations. Contact your administrator for assistance.
            ...
            <div className="space-y-6">
              {showForm && (
                <Card>
                  <CardHeader>
                    <CardTitle>{editingConfig ? "Edit" : "Add"} Fee Configuration</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <Form {...form}>
                              ...
                              <FormItem>
                                ...
                                <Select onValueChange={field.onChange} defaultValue={field.value}>
                                  ...
                                  <SelectContent>
                                    ...
                                    <SelectItem value="agency_banking">Agency Banking</SelectItem>
                              ...
                              <FormItem>
                                ...
                                <Select onValueChange={field.onChange} defaultValue={field.value}>
                                  ...
                                  <SelectContent>
                                    ...
                                    <SelectItem value="interbank_transfer">Interbank Transfer</SelectItem>
                          ...
                          <FormField
                            ...
                            render={({ field }) => (
                              <FormItem>
                                ...
                                <FormDescription>
                                  {form.watch("fee_type") === "percentage" ? "Percentage (0-100)" : "Amount in GHS"}
                                </FormDescription>
                                <FormMessage />
                              </FormItem>
                            )}
                        ...
                        <div className="flex gap-4">
                          <Button type="submit" disabled={isLoading}>
                            {isLoading ? (
                              ...
                              `${editingConfig ? "Update" : "Create"} Fee Configuration`
                            )}
                          </Button>
                          <Button
                            type="button"
                ...
                <CardContent>
                      ...
                      {feeConfigs.map((config) => (
                        <TableRow key={config.id}>
                          <TableCell className="font-medium">
                            {config.service_type.replace("_", " ").toUpperCase()}
                          </TableCell>
                          <TableCell>{config.transaction_type.replace("_", " ")}</TableCell>
                          <TableCell>
                            <Badge variant="outline">{config.fee_type}</Badge>
                          </TableCell>
                          <TableCell>
                            {config.fee_type === "percentage" ? `${config.fee_value}%` : `GHS ${config.fee_value}`}
                          </TableCell>
                  ...
                  {feeConfigs.length === 0 && (
                    <div className="text-center py-8">
                      <p className="text-muted-foreground">No fee configurations found</p>
                    </div>
                  )}
                </CardContent>
              </Card>
