...
import { Plus, Trash2, RefreshCw, Info, BookOpen, AlertCircle, Database } from "lucide-react"
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
export function ManualGLFloatMapping() {
    ...
    try {
      ...
      if (data.success) {
        console.log("✅ Data fetched successfully:", {
          floatAccounts: data.floatAccounts?.length || 0,
          glAccounts: data.glAccounts?.length || 0,
          mappings: data.mappings?.length || 0,
        })

        // Ensure all arrays are properly set
        setFloatAccounts(Array.isArray(data.floatAccounts) ? data.floatAccounts : [])
        setGLAccounts(Array.isArray(data.glAccounts) ? data.glAccounts : [])
        setMappings(Array.isArray(data.mappings) ? data.mappings : [])

        // Show specific messages for empty data
        if (data.glAccounts?.length === 0) {
          toast({
            title: "No GL Accounts Found",
            description: "GL accounts have been automatically created. Please refresh the page.",
            variant: "default",
          })
        }
      } else {
        ...
        if (data.schemaIssue) {
          toast({
            ...
            description: "GL accounts table needs to be fixed. Basic accounts have been created.",
            variant: "default",
          })
        } else {
        ...
        // Set empty arrays even on error to prevent undefined
        setFloatAccounts([])
        setGLAccounts([])
        setMappings([])
      }
      ...
      toast({
        title: "Error",
        description: error instanceof Error ? error.message : "Failed to load mapping data",
        variant: "destructive",
      })
  ...
  const getAvailableFloatAccounts = () => {
    if (mappingType === "main_account") {
      // For main accounts, each float can only have one mapping
      const mappedAccountIds = mappings
        .filter((m) => m.mapping_type === "main_account" && m.is_active)
        .map((m) => m.float_account_id)

      return floatAccounts.filter((account) => !mappedAccountIds.includes(account.id))
    } else {
      // For fee and commission accounts, all floats are available
  ...
  const getServiceLabel = (accountType: string, provider?: string, accountNumber?: string) => {
    let label = ""
      ...
      <Dialog open={isGuideDialogOpen} onOpenChange={setIsGuideDialogOpen}>
        <DialogContent className="sm:max-w-[800px] max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Understanding how General Ledger accounts connect to Float accounts and Financial Reports
            </DialogDescription>
          </DialogHeader>

          <div className="space-y-6">
            <div>
              <h3 className="font-semibold text-lg mb-3">🏦 What is GL-Float Mapping?</h3>
              <p className="text-sm text-muted-foreground mb-4">
                GL-Float mapping connects your operational float accounts (where you track service balances) to your
                accounting system's General Ledger accounts (for financial reporting).
              </p>

              <div className="bg-blue-50 p-4 rounded-lg">
                <h4 className="font-medium mb-2">Example: Mobile Money Service</h4>
                <div className="text-sm space-y-1">
                  <div>
                    <strong>Float Account:</strong> "MTN MoMo - Accra Branch" (Balance: GHS 50,000)
                  </div>
                  <div>
                    <strong>Main GL Account:</strong> "1003 - Mobile Money Cash" (Asset)
                  </div>
                  <div>
                    <strong>Fee GL Account:</strong> "4001 - Transaction Fee Revenue" (Revenue)
                  </div>
                  <div>
                    <strong>Commission GL Account:</strong> "4002 - MoMo Commission Revenue" (Revenue)
                  </div>
                </div>
              </div>
            </div>
            ...
            <div>
              <h3 className="font-semibold text-lg mb-3">🔗 Mapping Types Explained</h3>
              <div className="space-y-3">
                {mappingTypeOptions.map((type) => (
                  <div key={type.value} className="border rounded-lg p-3">
                    <div className="font-medium">{type.label}</div>
                    <div className="text-sm text-muted-foreground">{type.description}</div>
                    <div className="text-xs text-blue-600 mt-1">Example: {type.example}</div>
                  </div>
                ))}
              </div>
            </div>
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between">
          <div>
            ...
            <CardDescription>Create and manage manual mappings between float accounts and GL accounts</CardDescription>
          </div>
          <div className="flex gap-2">
            <Button onClick={() => setIsDebugDialogOpen(true)} variant="outline" size="sm">
              <Database className="h-4 w-4 mr-2" />
              Debug
            </Button>
            <Button onClick={() => setIsGuideDialogOpen(true)} variant="outline" size="sm">
            ...
            <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
              ...
              <DialogContent className="sm:max-w-[700px]">
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Map a float account to a general ledger account for automatic posting
                  </DialogDescription>
                </DialogHeader>
                ...
                <Alert>
                  ...
                  <AlertDescription>
                    ...
                    <br />• <strong>Main Account:</strong> Primary account for transaction amounts (1:1 mapping)
                    <br />• <strong>Fee Account:</strong> Account for transaction fees and charges (N:1 mapping)
                    <br />• <strong>Commission Account:</strong> Account for commission earnings (N:1 mapping)
                  </AlertDescription>
                </Alert>
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Select value={selectedFloatAccount} onValueChange={setSelectedFloatAccount}>
                      ...
                      <SelectContent>
                        {availableFloatAccounts.map((account) => (
                          <SelectItem key={account.id} value={account.id}>
                            <div>
                              <div className="font-medium">
                                {account.branch_name} -{" "}
                                {getServiceLabel(account.account_type, account.provider, account.account_number)}
                              </div>
                    ...
                    {availableFloatAccounts.length === 0 && (
                  ...
                  <div className="space-y-2">
                    <label className="text-sm font-medium">GL Account ({glAccounts.length} available)</label>
                        ...
                        {glAccounts.map((account) => (
                          <SelectItem key={account.id} value={account.id}>
                            <div>
                              ...
                              <div className="text-xs text-muted-foreground">
                                Type: {account.account_type} | Balance: {formatCurrency(account.balance)}
                              </div>
                            </div>
                          </SelectItem>
                        ))}
                    ...
                    {glAccounts.length === 0 && (
                      <div className="text-sm text-muted-foreground space-y-2">
                        <p>No GL accounts available. Basic GL accounts should have been created automatically.</p>
                        <Button onClick={fetchData} variant="outline" size="sm">
                ...
                <DialogFooter>
                  ...
                  <Button
                    ...
                    disabled={!selectedFloatAccount || !selectedGLAccount || glAccounts.length === 0}
                  >
                    Create Mapping
                  </Button>
                </DialogFooter>
            ...
            <>
              <div className="mb-4 grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="text-center p-4 bg-blue-50 rounded-lg">
                  <div className="text-2xl font-bold text-blue-600">{floatAccounts.length}</div>
                  <div className="text-sm text-blue-600">Float Accounts</div>
                </div>
                <div className="text-center p-4 bg-green-50 rounded-lg">
                  <div className="text-2xl font-bold text-green-600">{glAccounts.length}</div>
              ...
              {glAccounts.length === 0 && (
                <Alert className="mb-4">
                  ...
                  <AlertDescription>
                    <strong>No GL Accounts Found:</strong> Basic GL accounts should have been created automatically. If
                    you still see 0 GL accounts after refreshing, please check that the GL accounts table exists and has
                    data.
                    <br />
                  ...
                  {mappings.map((mapping) => (
                    <TableRow key={mapping.id}>
                      ...
                      <TableCell className="text-right">
                        {formatCurrency(mapping.float_account?.current_balance || 0)}
                      </TableCell>
                      <TableCell>
                        {mapping.gl_account?.account_code} - {mapping.gl_account?.account_name}
                      </TableCell>
                      <TableCell>
                        <Badge variant="outline">{mapping.gl_account?.account_type}</Badge>
                      </TableCell>
                      <TableCell className="text-right">{formatCurrency(mapping.gl_account?.balance || 0)}</TableCell>
                      <TableCell>
                        <Badge variant="secondary">{mapping.mapping_type.replace("_", " ").toUpperCase()}</Badge>
                      </TableCell>
                      <TableCell>
          ...
          {mappings.length === 0 && !loading && (
            <div className="text-center py-8">
              <div className="text-gray-500">
                No mappings found. Create your first mapping using the "Add Mapping" button.
              </div>
              {glAccounts.length === 0 && (
                <div className="mt-4">
                  <Alert>
                    <AlertCircle className="h-4 w-4" />
                    <AlertDescription>
                      <strong>Missing GL Accounts:</strong> Basic GL accounts should have been created automatically.
                      Try refreshing the page or check the debug information.
                    </AlertDescription>
                  </Alert>
                </div>
              )}
