...
import { RefreshCw, Plus, TrendingUp, DollarSign, Clock, CheckCircle } from "lucide-react"
...
export function CommissionDashboard() {
  ...
  const statistics = {
    totalAmount: commissions?.reduce((sum, c) => sum + (Number(c.amount) || 0), 0) || 0,
    totalCount: commissions?.length || 0,
    pendingAmount:
      commissions?.filter((c) => c.status === "pending")?.reduce((sum, c) => sum + (Number(c.amount) || 0), 0) || 0,
    pendingCount: commissions?.filter((c) => c.status === "pending")?.length || 0,
    paidAmount:
      commissions?.filter((c) => c.status === "paid")?.reduce((sum, c) => sum + (Number(c.amount) || 0), 0) || 0,
  ...
  const handleMarkPaid = async (commission: any) => {
    try {
      ...
      toast({
        title: "Error",
        description: error instanceof Error ? error.message : "Failed to mark commission as paid",
        variant: "destructive",
      })
    }
  }
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground">Track and manage partner commissions</p>
          </div>
          <BranchIndicator />
        </div>
        ...
        <Card className="border-red-200 bg-red-50">
          <CardHeader>
            <CardTitle className="text-red-800">Error Loading Commissions</CardTitle>
          </CardHeader>
          <CardContent className="text-red-700">
            <p>{error}</p>
            <Button variant="outline" onClick={refreshCommissions} className="mt-2">
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <BranchIndicator />
          <Button variant="outline" size="sm" onClick={refreshCommissions} disabled={isLoading}>
          ...
          <CardContent>
            {isLoading ? (
              ...
              <>
                ...
                <p className="text-xs text-muted-foreground">{statistics.totalCount} entries</p>
              </>
            )}
          </CardContent>
        </Card>
          ...
          <CardContent>
            {isLoading ? (
              ...
              <>
                ...
                <p className="text-xs text-muted-foreground">{statistics.pendingCount} pending</p>
              </>
            )}
          </CardContent>
        </Card>
          ...
          <CardContent>
            {isLoading ? (
              ...
              <>
                ...
                <p className="text-xs text-muted-foreground">{statistics.paidCount} completed</p>
              </>
            )}
          </CardContent>
        </Card>
            ...
            {isLoading ? (
              ...
              <>
                <div className="text-2xl font-bold">
                  {statistics.totalCount > 0 ? Math.round((statistics.paidCount / statistics.totalCount) * 100) : 0}%
                </div>
          ...
          <div className="grid gap-6 md:grid-cols-2">
            <Card>
              <CardHeader>
                ...
                <CardDescription>Commission amounts by service provider</CardDescription>
              </CardHeader>
              <CardContent>
              ...
              <CardContent>
                {isLoading ? (
                    ...
                    {commissions.slice(0, 5).map((commission) => (
                      <div key={commission.id} className="flex items-center justify-between p-3 border rounded-lg">
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">{commission.reference}</p>
                        </div>
                  ...
                  <div className="text-center py-8 text-muted-foreground">
                    ...
                    <p className="text-sm mt-1">Add your first commission to get started</p>
                  </div>
                )}
              </CardContent>
            </Card>
        ...
        <TabsContent value="analytics" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Detailed breakdown of commission data</CardDescription>
            </CardHeader>
            <CardContent>
              <CommissionChart data={commissions || []} />
            </CardContent>
      ...
      <AddCommissionDialog open={showAddDialog} onOpenChange={setShowAddDialog} onSuccess={handleAddCommission} />
    </div>
  )
}
