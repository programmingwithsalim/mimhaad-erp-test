...
const bulkEditSchema = z.object({
  ...
  amountAction: z.enum(["unchanged", "set", "increase", "decrease"]).default("unchanged"),
...
export function BulkEditDialog({ selectedCommissions, onSuccess, disabled = false }: BulkEditDialogProps) {
  ...
  const handleSubmit = async (values: BulkEditFormValues) => {
    ...
    if (values.amountAction !== "unchanged" && values.amountValue) {
      ...
      if (!isNaN(amountValue)) {
        ...
        const updatePromises = selectedCommissions.map(async (commission) => {
          ...
          return updateCommission(commission.id, { ...updates, amount: newAmount })
        })
    ...
    try {
      ...
      const results = await Promise.all(
        selectedCommissions.map((commission) => updateCommission(commission.id, updates)),
      )
      ...
      <DialogContent className="sm:max-w-[500px]">
        ...
        <div className="py-4">
          <p className="text-sm text-muted-foreground mb-4">
            Editing {selectedCommissions.length} commissions. Only fields you change will be updated.
          </p>
                    ...
                    <FormControl>
                      ...
                      >
                        <FormItem className="flex items-center space-x-3 space-y-0">
                          ...
                          <FormLabel className="font-normal">Leave unchanged</FormLabel>
                        </FormItem>
                        <FormItem className="flex items-center space-x-3 space-y-0">
                          <FormControl>
                            <RadioGroupItem value="set" />
                          </FormControl>
                          <FormLabel className="font-normal">Set to specific amount</FormLabel>
                        </FormItem>
                        <FormItem className="flex items-center space-x-3 space-y-0">
                          <FormControl>
                            <RadioGroupItem value="increase" />
                          </FormControl>
                          <FormLabel className="font-normal">Increase by percentage</FormLabel>
                        </FormItem>
                        <FormItem className="flex items-center space-x-3 space-y-0">
                          <FormControl>
                            <RadioGroupItem value="decrease" />
                          </FormControl>
                          <FormLabel className="font-normal">Decrease by percentage</FormLabel>
                        </FormItem>
                      </RadioGroup>
                    </FormControl>
                    <FormMessage />
