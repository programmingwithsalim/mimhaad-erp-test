...
import { DollarSign, TrendingUp, Activity, Building2, AlertTriangle, RefreshCw, Edit, Trash2 } from "lucide-react"
...
export function UniversalServiceLayout({
  ...
  const { statistics, floatAlerts, isLoading: statsLoading, refreshStatistics } = useServiceStatistics(serviceEndpoint)
  ...
  const fetchTransactions = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/${serviceEndpoint}/transactions?branchId=${user.branchId}`)
  ...
  const handleRefreshAll = async () => {
    await Promise.all([refreshCashTill(), refreshStatistics(), fetchTransactions()])
    toast({
      title: "Refreshed",
  ...
  const handleDeleteTransaction = async (transactionId: string) => {
    if (
      !confirm("Are you sure you want to delete this transaction? This will adjust the float balance and GL entries.")
    ) {
      return
    }
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Manage {serviceName.toLowerCase()} transactions and view real-time insights
          </p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">Today's Volume: {formatCurrency(statistics.todayVolume)}</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">{statistics.lowFloatAlerts} low balance alerts</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">{cashError ? "Error loading" : `Status: ${balanceStatus}`}</p>
          </CardContent>
        </Card>
      </div>
        ...
        <TabsContent value="history" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Recent {serviceName.toLowerCase()} transactions with edit and delete options
              </CardDescription>
            </CardHeader>
            <CardContent>
              {transactionsLoading ? (
