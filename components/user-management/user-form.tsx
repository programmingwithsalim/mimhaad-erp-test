...
import { Loader2, AlertCircle, Building2, User, Mail, Phone, Shield } from "lucide-react"
...
const AVAILABLE_ROLES = [
  ...
  { id: "Manager", name: "Manager", description: "Branch management and oversight" },
  { id: "Finance", name: "Finance", description: "Financial operations and reporting" },
...
export function UserForm({ user = null, branches = [], isLoadingBranches = false, onSubmit, onCancel }: UserFormProps) {
  ...
  const form = useForm<UserFormValues>({
    ...
    defaultValues: user
      ? {
          ...
          branchIds: user.branchIds || user.branches || [user.primaryBranchId || user.branch_id || ""],
          phone: user.phone || "",
  ...
  useEffect(() => {
    const subscription = form.watch((value, { name }) => {
      ...
      if (
        ...
        const newBranchIds = [...((value.branchIds as string[]) || []), value.primaryBranchId as string]
  ...
  useEffect(() => {
    if (user) {
      const userBranches = user.branchIds || user.branches || [user.primaryBranchId || user.branch_id]
  ...
  const handleBranchSelection = (branchId: string, isChecked: boolean) => {
    ...
    if (isChecked) {
      ...
      if (form.getValues("primaryBranchId") === branchId) {
        ...
        const newPrimaryBranch = newSelectedBranchIds.length > 0 ? newSelectedBranchIds[0] : ""
    ...
    <div className="max-w-4xl mx-auto p-6">
      <div className="mb-6">
        ...
        <p className="text-muted-foreground">
          {user ? "Update user information and permissions" : "Create a new user account with appropriate access"}
        </p>
      </div>
        ...
        <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormDescription>This will be used for login and system notifications</FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
              />
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Define user access level and system permissions</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <FormField
                control={form.control}
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormDescription>Role determines what features and data the user can access</FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
              />
                ...
                render={({ field }) => (
                  <FormItem className="flex flex-row items-center justify-between rounded-lg border p-4">
                    <div className="space-y-0.5">
                      ...
                      <FormDescription>Active accounts can log in and access the system</FormDescription>
                    </div>
                    <FormControl>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Assign user to branches and set primary location</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
              {isLoadingBranches ? (
                ...
                <Alert>
                  ...
                  <AlertDescription>
                    Failed to load branches. Please refresh the page or contact support.
                ...
                <Alert>
                  <AlertCircle className="h-4 w-4" />
                  <AlertDescription>
                    No branches found. Please create branches first before assigning users.
                    <Button
                      variant="link"
                      className="p-0 h-auto ml-2"
                      onClick={() => window.open("/dashboard/branch-management", "_blank")}
                    >
                      Manage Branches
                    </Button>
                  </AlertDescription>
                ...
                <>
                  {/* Branch Selection */}
                  <div>
                    <FormLabel className="text-base font-medium mb-3 block">Available Branches</FormLabel>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 max-h-64 overflow-y-auto border rounded-lg p-4">
                      {branches.map((branch) => {
                        ...
                        const isPrimary = form.getValues("primaryBranchId") === branch.id
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormDescription>The main branch where this user is primarily located</FormDescription>
                        <FormMessage />
                      </FormItem>
                    )}
                  />
                  ...
                  {selectedBranchIds.length > 0 && (
                    <div className="bg-muted/50 rounded-lg p-4">
                      <h4 className="font-medium mb-2">Selected Branches ({selectedBranchIds.length})</h4>
                      <div className="flex flex-wrap gap-2">
                        {selectedBranchIds.map((branchId) => {
                          const branch = branches.find((b) => b.id === branchId)
                          const isPrimary = form.getValues("primaryBranchId") === branchId
                          return (
                            <Badge key={branchId} variant={isPrimary ? "default" : "secondary"} className="text-xs">
