...
export function WithdrawalManagement() {
  const { withdrawals, loading, error, fetchWithdrawals, processWithdrawal, updateWithdrawal, deleteWithdrawal } =
    useWithdrawals()
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground">Manage E-Zwich withdrawal transactions</p>
        </div>
            ...
            {withdrawals.length === 0 ? (
              <TableRow>
                <TableCell colSpan={7} className="text-center py-8">
                  <div className="text-muted-foreground">No withdrawal transactions found</div>
                </TableCell>
              </TableRow>
            ) : (
              withdrawals.map((withdrawal) => (
                <TableRow key={withdrawal.id}>
                  <TableCell className="font-medium">{withdrawal.card_number}</TableCell>
                  <TableCell>
                    <div>
                  ...
                  <TableCell>GHS {withdrawal.amount?.toLocaleString() || "0.00"}</TableCell>
                  <TableCell>
                    <Badge className={getStatusColor(withdrawal.status)}>{withdrawal.status}</Badge>
                  </TableCell>
                  <TableCell>{format(new Date(withdrawal.transaction_date), "MMM d, yyyy")}</TableCell>
                  ...
                  <TableCell>
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        ...
                        <DropdownMenuItem onClick={() => handleDelete(withdrawal)} className="text-red-600">
      ...
      <Dialog open={showEditDialog} onOpenChange={setShowEditDialog}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>Update withdrawal transaction details</DialogDescription>
          </DialogHeader>
      ...
      <AlertDialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to delete this withdrawal transaction? This action cannot be undone.
              <br />
              <strong>Reference:</strong> {deletingWithdrawal?.reference}
              <br />
              <strong>Amount:</strong> GHS {deletingWithdrawal?.amount?.toLocaleString()}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
