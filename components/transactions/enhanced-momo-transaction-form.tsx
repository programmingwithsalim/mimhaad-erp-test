...
export function EnhancedMomoTransactionForm({ onSuccess, momoFloats }: EnhancedMomoTransactionFormProps) {
  ...
  const onSubmit = async (data: MomoTransactionFormData) => {
    ...
    try {
      // CRITICAL: Ensure we're sending UUIDs, not usernames
      const transactionData = {
        ...
        branch_id: user.branchId, // This should be a UUID
        // Don't send processed_by in the body - let the API handle it
      }

      console.log("🔄 [MOMO] Submitting transaction with user context:", {
        userId: user.id,
        userName: user.name,
      ...
      console.log("🔄 [MOMO] Transaction data:", transactionData)

      const response = await fetch("/api/momo/transaction", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          // Send user context in headers as UUIDs
          "x-user-id": user.id, // UUID
          "x-user-name": user.name || user.username, // Display name
          "x-user-role": user.role,
          "x-branch-id": user.branchId, // UUID
