...
export function EditEZwichTransactionDialog({
  ...
  const onSubmit = async (data: EditTransactionFormValues) => {
    ...
    setIsLoading(true)
    try {
      const response = await fetch(`/api/e-zwich/transactions/${transaction.id}`, {
        method: "PUT",
        headers: {
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <Input id="customer_phone" {...form.register("customer_phone")} placeholder="Enter phone number" />
              {form.formState.errors.customer_phone && (
                <p className="text-sm text-red-500 mt-1">{form.formState.errors.customer_phone.message}</p>
              )}
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              {form.formState.errors.transaction_amount && (
                <p className="text-sm text-red-500 mt-1">{form.formState.errors.transaction_amount.message}</p>
              )}
            ...
            <div>
              ...
              <Input id="fee_amount" type="number" step="0.01" {...form.register("fee_amount")} placeholder="0.00" />
              {form.formState.errors.fee_amount && (
                <p className="text-sm text-red-500 mt-1">{form.formState.errors.fee_amount.message}</p>
              )}
              <p className="text-xs text-muted-foreground mt-1">Original: {formatCurrency(transaction.fee_amount)}</p>
            </div>
          </div>
