...
import { Mail, Bell, CheckCircle, XCircle, Clock, Send, Settings, TestTube, AlertTriangle } from "lucide-react"
...
export function NotificationTestCenter() {
  ...
  const [testMessage, setTestMessage] = useState("This is a test notification from your FinTech platform.")
  ...
  const testScenario = async () => {
    ...
    try {
      ...
      addTestResult({
        type: `Scenario: ${scenarioType}`,
        status: "error",
        message: error instanceof Error ? error.message : "Scenario test failed",
        details: error,
      })
      toast({
        title: "Error",
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Test individual notification components</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <div className="flex gap-2">
              <Button onClick={testBasicEmail} disabled={loading || !testEmail} className="flex-1">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Test real-world notification scenarios</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="space-y-2">
              ...
              <Select value={scenarioType} onValueChange={setScenarioType}>
                ...
                <SelectContent>
                  <SelectItem value="transaction_alert">High-Value Transaction Alert</SelectItem>
                  <SelectItem value="low_balance">Low Balance Warning</SelectItem>
            ...
            <Button onClick={testScenario} disabled={loading || !testEmail} className="w-full">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Recent notification test results (last 10)</CardDescription>
        </CardHeader>
        <CardContent>
              ...
              {testResults.map((result) => (
                <div key={result.id} className="flex items-start gap-3 p-3 border rounded-lg">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-muted-foreground mt-1">{result.timestamp.toLocaleString()}</p>
                  </div>
                </div>
              ))}
            </div>
