      ...
      for (const floatAccount of floatAccounts) {
        try {
          ...
          for (const mapping of mappings) {
            ...
            if (success) {
              results.success++
              results.details.push(`Mapped ${floatAccount.account_name} (${mapping.type}) to ${mapping.glAccountCode}`)
            } else {
              results.failed++
    ...
    try {
      ...
      for (const mapping of mappings) {
        try {
          ...
          result.errors.push(`Failed to sync ${mapping.float_account_id}: ${error.message}`)
        }
      }
