...
export async function createExpense(data: {
  ...
  try {
    ...
    const result = await sql`
      ...
      VALUES (
        ${referenceNumber}, ${data.branch_id}, ${data.expense_head_id}, ${data.amount}, 
...
export async function approveExpense(id: string, approver_id: string, comments?: string): Promise<Expense | null> {
  try {
    ...
    if (result.length === 0) {
      console.log("No expense found to approve or expense not in pending status")
      return null
    }
...
export async function rejectExpense(id: string, rejector_id: string, reason: string): Promise<Expense | null> {
  try {
    ...
    if (result.length === 0) {
      console.log("No expense found to reject or expense not in pending status")
      return null
    }
...
export async function markExpensePaid(id: string, payer_id: string): Promise<Expense | null> {
  try {
    ...
    if (result.length === 0) {
      console.log("No expense found to mark as paid or expense not in approved status")
      return null
    }
...
export async function deleteExpense(id: string): Promise<boolean> {
  try {
    ...
    const success = result.rowCount > 0 || result.count > 0 || result.length === 0
...
export async function getExpenseStatistics(filters?: ExpenseFilters): Promise<ExpenseStatistics> {
  try {
    ...
    const paymentSourceResult = await sql`
      SELECT 
        payment_source,
        COUNT(*) as count,
