...
import { getAgencyBankingTransactions } from "./agency-banking-service"
  ...
  try {
    ...
    for (const transaction of momoTransactions) {
      try {
        ...
        const existingGLTransactions = await GLDatabase.getGLTransactionsBySourceId(transaction.id)
        ...
        const { debitAccount, creditAccount } = await getGLAccountsForMoMoTransaction(transaction.type)

        if (!debitAccount || !creditAccount) {
          throw new Error(`GL accounts not found for MoMo transaction type: ${transaction.type}`)
        }
        ...
        console.error(`Error processing MoMo transaction ${transaction.id}:`, error)
    ...
    if (syncStatus.recordsFailed > 0) {
      syncStatus.status = syncStatus.recordsSucceeded > 0 ? "partial" : "failed"
    }
    ...
    await GLDatabase.addSyncLogEntry({
      ...
      error: syncStatus.recordsFailed > 0 ? `Failed to sync ${syncStatus.recordsFailed} transactions` : undefined,
    })
...
async function getGLAccountsForMoMoTransaction(type: string): Promise<{ debitAccount: any; creditAccount: any }> {
...
export async function syncAgencyBankingTransactions(): Promise<SyncStatus> {
  ...
  try {
    ...
    const agencyTransactions = await getAgencyBankingTransactions({ status: "completed" })
    ...
    for (const transaction of agencyTransactions) {
      try {
        ...
        // Get GL accounts based on transaction type
        const { debitAccount, creditAccount } = await getGLAccountsForAgencyTransaction(transaction.type)

        if (!debitAccount || !creditAccount) {
          throw new Error(`GL accounts not found for Agency Banking transaction type: ${transaction.type}`)
        }
        ...
        if (transaction.fee > 0) {
          const { debitAccount: feeDebitAccount, creditAccount: feeCreditAccount } = await getGLAccountsForAgencyFee()
        ...
        console.error(`Error processing Agency Banking transaction ${transaction.id}:`, error)
...
async function getGLAccountsForAgencyTransaction(type: string): Promise<{ debitAccount: any; creditAccount: any }> {
  switch (type) {
    case "deposit":
      return {
        debitAccount: await GLDatabase.getGLAccountByCode("1001"), // Cash
...
async function getGLAccountsForAgencyFee(): Promise<{ debitAccount: any; creditAccount: any }> {
  return {
    debitAccount: await GLDatabase.getGLAccountByCode("2003"), // Bank Partner Liability
  ...
  try {
    ...
    for (const commission of commissions) {
      try {
        ...
        const existingGLTransactions = await GLDatabase.getGLTransactionsBySourceId(commission.id)
        ...
        const { debitAccount, creditAccount } = await getGLAccountsForCommission(commission.source)

        if (!debitAccount || !creditAccount) {
          throw new Error(`GL accounts not found for Commission source: ${commission.source}`)
        }
    ...
    await GLDatabase.addSyncLogEntry({
      ...
      affectedRecords: syncStatus.recordsSucceeded,
      error: syncStatus.recordsFailed > 0 ? `Failed to sync ${syncStatus.recordsFailed} commissions` : undefined,
    })
...
// Get GL accounts for Commission
async function getGLAccountsForCommission(source: string): Promise<{ debitAccount: any; creditAccount: any }> {
  // This is a simplified implementation. In a real system, you would have a more sophisticated
  // mapping system that could be configured by users.
  ...
  try {
    ...
    for (const expense of expenses) {
      try {
        ...
        const existingGLTransactions = await GLDatabase.getGLTransactionsBySourceId(expense.id)
        ...
        const { debitAccount, creditAccount } = await getGLAccountsForExpense(expense.expenseHeadId)

        if (!debitAccount || !creditAccount) {
          throw new Error(`GL accounts not found for Expense head: ${expense.expenseHeadId}`)
        }
    ...
    await GLDatabase.addSyncLogEntry({
      ...
      affectedRecords: syncStatus.recordsSucceeded,
      error: syncStatus.recordsFailed > 0 ? `Failed to sync ${syncStatus.recordsFailed} expenses` : undefined,
    })
...
// Get GL accounts for Expense
async function getGLAccountsForExpense(expenseHeadId: string): Promise<{ debitAccount: any; creditAccount: any }> {
