...
export class GLDatabase {
  ...
  static async createGLAccount(account: {
    ...
    try {
      ...
      if (existing) {
        console.log(`GL account ${account.code} already exists, returning existing account`)
        return existing
      }
      ...
      const newAccount = await sql`
        ...
        VALUES (${accountId}, ${account.code}, ${account.name}, ${normalizedType}, ${null}, 0, true)
   ...
   */
  static async getGLTransactionsBySourceId(sourceTransactionId: string): Promise<GLTransaction[]> {
    try {
      ...
      console.error(`Error fetching GL transactions for source ID ${sourceTransactionId}:`, error)
      return []
    }
  }
   ...
   */
  static async createGLTransaction(transaction: Omit<GLTransaction, "id" | "createdAt">): Promise<GLTransaction> {
    try {
      ...
      const newTransaction = await sql`
        ...
        VALUES (
          ...
          ${transaction.sourceTransactionId}, ${transaction.sourceTransactionType},
          ${transaction.description}, ${transaction.status || "pending"}, ${transaction.createdBy},
      ...
      for (const entry of transaction.entries) {
        console.log(`Creating journal entry for account: ${entry.accountCode} (${entry.accountId})`)
   ...
   */
  static async postGLTransaction(transactionId: string, postedBy: string): Promise<void> {
    try {
      await sql`
   ...
   */
  static async addSyncLogEntry(entry: Omit<SyncLogEntry, "id" | "createdAt">): Promise<void> {
    try {
      await sql`
        INSERT INTO gl_sync_logs (module, operation, status, details, error)
        VALUES (${entry.module}, ${entry.operation}, ${entry.status}, ${entry.details}, ${entry.error || null})
      `
    } catch (error) {
    ...
    for (const account of requiredAccounts) {
      ...
      if (!existing) {
        try {
          ...
          console.log(`Created missing GL account: ${account.code} - ${account.name}`)
        } catch (error) {
...
export async function updateGLAccount(
  ...
  try {
    ...
    const data = await readJsonFile<{ accounts: GLAccount[] }>(GL_DATABASE_PATH)
    ...
    if (updates.code && updates.code !== accounts[accountIndex].code) {
      const existingAccount = accounts.find((account) => account.code === updates.code)
      if (existingAccount) {
...
async function updateAccountBalanceCode(accountId: string, newCode: string): Promise<void> {
  try {
    const data = await readJsonFile<{ balances: GLAccountBalance[] }>(GL_BALANCES_PATH)
    const balances = data.balances || []

    const balanceIndex = balances.findIndex((balance) => balance.accountId === accountId)
    if (balanceIndex === -1) {
      return
    }
    ...
    console.error(`Error updating account balance code for ${accountId}:`, error)
    throw error
  }
}

// Get account balance
export async function getAccountBalance(accountId: string): Promise<GLAccountBalance | null> {
  try {
    await initGLDatabaseFiles()
...
export async function getAccountBalanceByCode(accountCode: string): Promise<GLAccountBalance | null> {
  try {
    await initGLDatabaseFiles()
    ...
    return balances.find((balance) => balance.accountCode === accountCode) || null
  } catch (error) {
    console.error(`Error getting account balance for code ${accountCode}:`, error)
    return null
  }
}
...
export async function getGLTransactions(filters?: {
  ...
  try {
    ...
    const data = await readJsonFile<{ transactions: GLTransaction[] }>(GL_TRANSACTIONS_PATH)
    ...
    if (filters) {
      ...
      if (filters.sourceModule) {
        transactions = transactions.filter((transaction) => transaction.sourceModule === filters.sourceModule)
      }

      if (filters.sourceTransactionType) {
        transactions = transactions.filter(
          (transaction) => transaction.sourceTransactionType === filters.sourceTransactionType,
        )
      }

      if (filters.startDate) {
        transactions = transactions.filter((transaction) => transaction.date >= filters.startDate)
      }

      if (filters.endDate) {
        transactions = transactions.filter((transaction) => transaction.date <= filters.endDate)
      }

      if (filters.createdBy) {
        transactions = transactions.filter((transaction) => transaction.createdBy === filters.createdBy)
      }
    }
...
export async function getGLTransactionById(id: string): Promise<GLTransaction | null> {
  try {
    await initGLDatabaseFiles()
...
export async function getSyncLogs(
  ...
  try {
    ...
    const data = await readJsonFile<{ logs: GLSyncLogEntry[] }>(GL_SYNC_LOG_PATH)
    let logs = data.logs || []
...
export async function getGLAccountTree(): Promise<Record<string, any>[]> {
  try {
    ...
    // Add type groups
    const typeOrder: GLAccountType[] = ["Asset", "Liability", "Equity", "Revenue", "Expense"]
...
function buildAccountTree(accounts: GLAccount[], parentId?: string): Record<string, any>[] {
...
export async function getTrialBalance(asOfDate?: string): Promise<{
  ...
  try {
    const accounts = await getAllGLAccounts()
    const balances = await readJsonFile<{ balances: GLAccountBalance[] }>(GL_BALANCES_PATH)
