    ...
    if (userData.branchIds && userData.branchIds.length > 0) {
      for (const branchId of userData.branchIds) {
        try {
          ...
          console.warn(`Failed to assign branch ${branchId} to user ${newUser.id}:`, branchError)
      ...
      if (userData.branchIds.length > 0) {
        for (const branchId of userData.branchIds) {
          try {
            ...
            console.warn(`Failed to assign branch ${branchId} to user ${id}:`, branchError)
            // Continue with other branches even if one fails
          }
        }
      }
...
export async function changeUserPassword(id: string, currentPassword: string, newPassword: string): Promise<boolean> {
  try {
