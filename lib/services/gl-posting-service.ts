...
export class GLPostingService {
  static async createAndPostTransaction(
    ...
    try {
      ...
      for (const entry of transactionData.entries) {
        ...
        await sql`
          INSERT INTO gl_journal_entries (
            ...
            ${entryId}, ${glTransactionId}, ${entry.accountId}, ${entry.accountCode},
      ...
      if (autoPost) {
        await this.updateAccountBalances(transactionData.entries, transactionData.date)
      }
  ...
  static async createCommissionGLEntries(params: {
    ...
    try {
      const receivableAccount = await this.getOrCreateGLAccount("1200", "Commission Receivable", "Asset")
      const revenueAccount = await this.getOrCreateGLAccount("4100", "Commission Revenue", "Revenue")
  ...
  static async createExpenseGLEntries(params: {
    ...
    try {
      const expenseAccount = await this.getOrCreateGLAccount("6000", "General Expenses", "Expense")
      const payableAccount = await this.getOrCreateGLAccount("2200", "Accounts Payable", "Liability")
  ...
  static async createMoMoGLEntries(params: {
    ...
    try {
      ...
      const cashAccount = await this.getOrCreateGLAccount("1001", "Cash", "Asset")
      const customerLiabilityAccount = await this.getOrCreateGLAccount("2001", "Customer Liability", "Liability")
      ...
      if (params.type === "cash-in") {
        entries.push({
          ...
          metadata: { transactionId: params.transactionId, provider: params.provider },
        })
   ...
   */
  private static async updateAccountBalances(entries: GLEntry[], date: string): Promise<void> {
