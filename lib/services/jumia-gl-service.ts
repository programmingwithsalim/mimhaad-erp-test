...
const GL_ACCOUNTS = {
  ...
  JUMIA_COMMISSION_REVENUE: {
    code: "4020-004",
    name: "Jumia Commission Revenue",
  },
...
async function checkGLSchema(): Promise<{
  exists: boolean;
  hasColumns: boolean;
}> {
  try {
    ...
    console.log(
      `GL schema check: table exists=${tableCheck[0]?.exists}, columns found=${columnCheck.length}/3`
    );
...
async function ensureGLAccounts(): Promise<boolean> {
  try {
    ...
    if (!schemaStatus.exists || !schemaStatus.hasColumns) {
      console.warn(
        "GL accounts table not properly configured. Schema status:",
        schemaStatus
      );
      return false;
    }
    ...
    console.log(
      `Creating ${accounts.length} GL accounts if they don't exist...`
    );
    ...
    for (const account of accounts) {
      try {
        ...
        `;
        console.log(
          `✓ Ensured GL account exists: ${account.code} - ${account.name}`
        );
      } catch (accountError) {
...
export async function createJumiaPackageReceiptGLEntries(
  ...
  console.log(
    `Creating GL entries for Jumia package receipt: ${transactionId}`
  );
  // No GL posting for package receipt
...
export async function createJumiaPODCollectionGLEntries(
  ...
  console.log(
    `Creating GL entries for Jumia POD collection: ${transactionId}, amount: ${amount}`
  );
  ...
  const glEntries = [
    ...
    {
      account_code: GL_ACCOUNTS.JUMIA_RECEIVABLES.code,
      account_name: GL_ACCOUNTS.JUMIA_RECEIVABLES.name,
  ...
  if (glAccountsReady) {
    try {
      ...
      console.log(
        `  Total Debits: ${glEntries.reduce(
          (sum, entry) => sum + (entry.debit_amount || 0),
          0
        )}`
      );
      console.log(
        `  Total Credits: ${glEntries.reduce(
      ...
      console.log(
        `✓ GL entries posted successfully for transaction ${transactionId}`
      );
    } catch (error) {
    ...
    console.warn(
      "GL schema not ready. Skipping GL posting for transaction:",
      transactionId
    );
  }
  ...
  await auditLogger.log({
    ...
    details: {
      ...
      total_debits: glEntries.reduce(
        (sum, entry) => sum + (entry.debit_amount || 0),
        0
      ),
      total_credits: glEntries.reduce(
        (sum, entry) => sum + (entry.credit_amount || 0),
        0
      ),
    },
...
export async function createJumiaSettlementGLEntries(
  ...
  console.log(
    `Creating GL entries for Jumia settlement: ${transactionId}, amount: ${amount}`
  );
  const glAccountsReady = await ensureGLAccounts();

  const paymentAccount =
    paymentMethod === "cash"
      ? GL_ACCOUNTS.CASH_IN_TILL
      : GL_ACCOUNTS.BANK_ACCOUNT;
  ...
  if (glAccountsReady) {
    try {
      ...
      await postGLEntries(glEntry, userId);
      console.log(
        `✓ GL entries posted successfully for settlement ${transactionId}`
      );
    } catch (error) {
      console.error("GL posting failed (non-critical):", error);
    }
...
async function postGLEntries(glEntry: JumiaGLEntry, userId: string) {
  try {
    console.log(
      `Posting GL entries for transaction ${glEntry.transaction_id}...`
    );
    ...
    const journalTableName =
      journalTableCheck[0]?.table_name || "gl_journal_entries";
    ...
    for (const entry of glEntry.gl_entries) {
      ...
      console.log(
        `✓ GL entry created: ${entry.account_code} - Debit: ${
          entry.debit_amount || 0
      ...
      try {
        ...
        console.warn(
          "Could not update account balance (table may not exist):",
          balanceError
        );
      }
    }
    ...
    console.log(
      `✓ All GL entries posted successfully for transaction ${glEntry.transaction_id}`
    );
  } catch (error) {
