...
export class EnhancedDashboardService {
  ...
  private static async getServiceTransactionStats(
    ...
    try {
      ...
      if (branchId && userRole !== "admin") {
        if (hasFees) {
          if (tableName === "momo_transactions") {
            ...
            `
          } else if (tableName === "agency_banking_transactions") {
            todayResult = await sql`
              SELECT 
                COUNT(*) as transaction_count,
                COALESCE(SUM(amount), 0) as total_volume,
              ...
              WHERE DATE(created_at) = CURRENT_DATE
                AND branch_id = ${branchId}
            `
          } else if (tableName === "e_zwich_transactions") {
            todayResult = await sql`
              SELECT 
                COUNT(*) as transaction_count,
                COALESCE(SUM(amount), 0) as total_volume,
          ...
          if (tableName === "power_transactions") {
            todayResult = await sql`
              SELECT 
                COUNT(*) as transaction_count,
                COALESCE(SUM(amount), 0) as total_volume,
              ...
              WHERE DATE(created_at) = CURRENT_DATE
                AND branch_id = ${branchId}
            `
          } else if (tableName === "jumia_transactions") {
            todayResult = await sql`
              SELECT 
                COUNT(*) as transaction_count,
                COALESCE(SUM(amount), 0) as total_volume,
    ...
    try {
      ...
      for (const service of services) {
        try {
          ...
          const serviceData = await this.getServiceTransactionStats(service.name, service.table, userRole, branchId)
          totalTransactions += Number(serviceData.transaction_count)
          totalRevenue += Number(serviceData.total_volume)
          totalFees += Number(serviceData.total_fees)
        } catch (error) {
    ...
    try {
      ...
      if (agencyExists[0]?.exists) {
        try {
          ...
          const hasPartnerBank = await this.checkColumnExists("agency_banking_transactions", "partner_bank")
          const hasBankName = await this.checkColumnExists("agency_banking_transactions", "bank_name")

          const bankColumn = hasPartnerBank ? "partner_bank" : hasBankName ? "bank_name" : "'Unknown'"
          ...
          if (branchId && userRole !== "admin") {
            agencyResult = await sql`
              SELECT 
                ...
                status,
                created_at,
                COALESCE(${sql(bankColumn)}, 'Unknown') as provider,
      ...
      if (jumiaExists[0]?.exists) {
        try {
          ...
          console.error("Error fetching Jumia transactions:", error)
        }
      }
    } catch (error) {
  ...
  private static async getAlerts(userRole: string, branchId?: string) {
    ...
    try {
      ...
      try {
        // Check which column exists for account name
        const hasAccountName = await this.checkColumnExists("float_accounts", "account_name")
        const hasProviderName = await this.checkColumnExists("float_accounts", "provider_name")

        const nameColumn = hasAccountName ? "account_name" : hasProviderName ? "provider_name" : "'Unknown'"

        let lowBalanceResult
        if (branchId && userRole !== "admin") {
          lowBalanceResult = await sql`
            SELECT ${sql(nameColumn)} as account_name, current_balance, account_type
            FROM float_accounts
          ...
          lowBalanceResult = await sql`
            SELECT ${sql(nameColumn)} as account_name, current_balance, account_type, branch_id
            FROM float_accounts
            WHERE current_balance < 5000
          `
        }
